...
Функция ПоМэппингу(GUID, ВнешняяСистема) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Рег.СтатьяДДС КАК СтатьяДДС
		|ИЗ
		|	РегистрСведений.ксп_МэппингСправочникСтатьиДДС КАК Рег
		|ГДЕ
		|	Рег.GUID = &GUID
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Возврат ВыборкаДетальныеЗаписи.СтатьяДДС;
	КонецЦикла;
...
Процедура ДобавитьГУИД(GUID, Наименование, ВнешняяСистема) Экспорт
	...
	НЗ = РегистрыСведений.ксп_МэппингСправочникСтатьиДДС.СоздатьНаборЗаписей();
...
//	Параметр1 	- Тип1 - 
//
Процедура ДобавитьЗапись(GUID, Наименование, ВнешняяСистема, СтатьяДДС) Экспорт
